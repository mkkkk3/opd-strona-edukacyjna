...
import { mainAlgorithmTree, findNodeById } from "../../helpers/tree";
...
function SpecificAlgorithmsTopic() {
  const router = useRouter();
  const { algorytmId } = router.query;

  const node =
    typeof algorytmId === "string"
      ? findNodeById(mainAlgorithmTree, algorytmId)
      : null;
          ...
          <div className="flex flex-col md:flex-row w-full">
            ...
            >
              ...
              <div className="mt-4 md:max-h-[calc(100vh-8rem)] md:overflow-y-auto md:pr-1">
                <nav aria-label="Nawigacja po drzewie algorytmów">
                  <ul className="space-y-3">
                    {mainAlgorithmTree.theory.map((topic) => (
                      <li key={topic.id}>
                        <button
                          type="button"
                          onClick={() => router.push(`/algorytmy/${topic.id}`)}
                          className="
                            w-full text-left
                            ...
                            {topic.content.map((section) => (
                              <li key={section.id}>
                                <button
                                  type="button"
                                  onClick={() =>
                                    router.push(`/algorytmy/${section.id}`)
                                  }
                                  className="
                                    w-full text-left
...
export default SpecificAlgorithmsTopic;
